...
def JoyCallback(msg):
   ...
   # print("X: \t {}".format(x))
    ...
    if start and not controlBotRunning and not startPreviouslyPressed:
        controlBotRunning = 1
        global xterm
        xterm = Popen(["xterm", "-e", "rosrun rosnet controlbot_stop.py"])
...
def TwistCallback(msg):
    ...
    if (msg.angular.z > 0.10):
        joystickInput = [1, 0, 0]
    elif (msg.angular.z < -0.10):
        joystickInput = [0, 0, 1]
...
def main():
    ...
    rospy.Subscriber("/camera/image", Image, ImageCallback,queue_size = 1, buff_size=2**24)
    rospy.spin()
